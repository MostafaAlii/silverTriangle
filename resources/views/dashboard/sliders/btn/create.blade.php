                    ...
                    <div class="tab round shadow">
                        <ul class="nav nav-tabs" role="tablist">
                            @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                <li class="nav-item">
                                    <a class="nav-link @if (app()->getLocale() == $localeCode) active show @endif"
                                        id="{{ $localeCode }}-tab" data-toggle="tab" href="#{{ $localeCode }}"
                                        role="tab" aria-controls="{{ $localeCode }}" aria-selected="true">
                        ...
                        <div class="tab-content">
                            @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                ...
                                <div class="tab-pane fade @if (app()->getLocale() == $localeCode) active show @endif"
                                    id="{{ $localeCode }}" role="tabpanel" aria-labelledby="{{ $localeCode }}-tab">
                                    <div class="form-group row">
                                        <div class="input-group mb-3 col-md-6">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text"
                                                    id="basic-addon1">{{ trans('slider.name') }}</span>
                                            </div>
                                            <input type="text" class="form-control"
                                                placeholder="{{ trans('slider.name') . ' ' . $properties['native'] }}"
                                                value="" id="name" name="{{ $localeCode }}[name]"
                                    ...
                                    <div class="form-group row">
                                        <div class="col-md-6">
                                            <label>{{ trans('slider.description') . ' ' . $properties['native'] }}
                                            </label>
                    ...
                    <div class="form-group p-1">
                        ...
                        <select name="status" class="form-control">
                            <option selected disabled>
                                {{ trans('general.choose') . ' ' . $title . ' ' . trans('general.status') }}...</option>
